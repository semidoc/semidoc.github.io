...
[Lempel-Ziv (LZ in short) algorithms](https://en.wikipedia.org/wiki/LZ77_and_LZ78) 
refer to a bunch of lossless
compression techniques introduced by 
[Abraham Lempel](https://en.wikipedia.org/wiki/Abraham_Lempel) and 
[Jacob Ziv](https://en.wikipedia.org/wiki/Jacob_Ziv) that
are used as key ingredients in various places such as 
*[deflate](https://en.wikipedia.org/wiki/DEFLATE)*,
*[gif](https://en.wikipedia.org/wiki/GIF)*, 
*[gzip](https://en.wikipedia.org/wiki/Gzip)*, etc.  
The version we consider is LZ'78, and works by
cutting the word $$w$$ we want to compress into substrings, called
blocks, $$w = m_1m_2\dots m_k $$, such that each block $$m_i$$, except
maybe the last one, is an extension of a previous block, that is:
$$m_i = m_j.a $$ for some letter $$a$$. 

// Peut-être mettre un coup d'exemple déjà ici. en expliquant les deux-trois 
premiers blocs.

It is not hard to see that
there is a unique cut that satisfies this property. Then, the
compression algorithm LZ'78 encodes each block $$m_i$$ as a *pointer*
to its *predecessor* $$m_j$$ together with the letter $$a$$.

For example, the word $$w = 00010110100001 $$ is cut as
...
It can be shown that the order of the compression size
 
// à définir ?

is
$$\Theta(k\log k)$$ where $$k$$ is the number of blocks.  The
*dictionnary* of a word $$w$$, written $$D(w)$$ will be the set of all
the blocks $$m_i$$. With this notation in hand, the size of the
compression can be rephrased as $$\Theta(|D(w)| \log |D(w)|) $$. A
word is said to be *incompressible* if the size of the compression is
of the order of the size of the word. Equivalently, a word is
incompressible if $$|D(w)|= \Theta(\frac{|w|}{\log |w|})$$.
